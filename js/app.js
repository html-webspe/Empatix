(() => {
    ...
    function toggleHeaderButton() {
        ...
        if (!headerBtn || !menuContent || !headerInner) return;
        if (document.documentElement.classList.contains("menu-open")) menuContent.appendChild(headerBtn); else headerInner.appendChild(headerBtn);
    }
    ...
    const firstSlider = document.querySelector(".slider-about--1"), secondSlider = document.querySelector(".slider-about--2");
    function initializeSliders() {
        if (firstSlider && secondSlider) {
            let allSlides = firstSlider.querySelectorAll(".swiper-slide"), lastSlideIndex = allSlides.length - 1;
            if (allSlides.length > 1) {
                var swiperFirst = new swiper_core_Swiper(firstSlider, {
                    slidesPerView: "auto",
                    spaceBetween: 15,
                    speed: 7e3,
                    initialSlide: 0,
                ...
                let swiperSecond = new swiper_core_Swiper(secondSlider, {
                    slidesPerView: "auto",
                    spaceBetween: 15,
                    speed: 7e3,
    ...
    function resizableSwiper(breakpoint, swiperClass, swiperSettings) {
        ...
        function filterSlides(category) {
            ...
            slides.forEach((slide => {
                const slideCategory = slide.getAttribute("data-filters-special");
                if (category === "*" || slideCategory === category) {
        ...
        filterButtons.forEach((button => {
            button.addEventListener("click", (function() {
                const category = this.getAttribute("data-filters-special");
    ...
    window.addEventListener("load", (function(e) {
        initializeSliders();
        resizableSwiper("(min-width: 600px)", ".slider-special", {
            modules: [ Pagination ],
            spaceBetween: 15,
            slidesPerView: 2,
            freeMode: true,
            pagination: {
                el: ".slider-special__pagination",
                clickable: true,
    ...
    function headerScroll() {
        ...
        document.addEventListener("windowScroll", (function(e) {
            ...
            if (scrollTop >= startPoint) {
                ...
                if (headerShow && !scrollEventTriggered) {
    ...
    document.querySelectorAll("[data-work]").forEach((parent => {
        const activeItem = parent.querySelector("[data-work-item].active");
        const img = parent.querySelector("[data-work-preview]");
        const link = parent.querySelector("[data-work-link]");
        if (activeItem) {
            updateWorkImage(activeItem, img);
            updateWorkLink(activeItem, link);
        }
    }));
    document.addEventListener("click", (function(event) {
        const clickedItem = event.target.closest("[data-work-item]");
        if (!clickedItem) return;
        const parent = clickedItem.closest("[data-work]");
        if (!parent) return;
        const img = parent.querySelector("[data-work-preview]");
        const link = parent.querySelector("[data-work-link]");
        parent.querySelectorAll("[data-work-item]").forEach((el => el.classList.remove("active")));
        clickedItem.classList.add("active");
        updateWorkImage(clickedItem, img);
        updateWorkLink(clickedItem, link);
    }));
    function updateWorkImage(activeItem, img) {
        if (!img) return;
        const newImage = activeItem.getAttribute("data-work-image");
        const randomDirection = [ "translateX(-100%)", "translateX(100%)", "translateY(-100%)", "translateY(100%)" ].sort((() => Math.random() - .5))[0];
        img.style.opacity = "0";
    ...
    function updateWorkLink(activeItem, link) {
        if (!link) return;
        const newLink = activeItem.getAttribute("data-work-links");
        if (newLink) {
            link.href = newLink;
    ...
    function updateActiveLink() {
        ...
        sections.forEach((section => {
            ...
            if (scrollPosition >= sectionTop && scrollPosition < sectionTop + sectionHeight) {
                ...
                if (targetLink) {
                    scrollButtons.forEach((btn => btn.classList.remove("link-active")));
    ...
    tl_prices.from(".block-prices__item", {
        x: -50,
        opacity: 0,
        width: "auto",
        duration: .8,
